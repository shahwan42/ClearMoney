...
func setupVirtualAccountTest(t *testing.T) (*VirtualAccountRepo, models.Institution, models.Account) {
	t.Helper()
	...
	return NewVirtualAccountRepo(db), inst, acc
}
...
func TestVirtualAccountRepo_GetTotalExcludedBalance(t *testing.T) {
	repo, _, acc := setupVirtualAccountTest(t)
	db := testutil.NewTestDB(t)
	...
	testutil.CreateVirtualAccount(t, db, models.VirtualAccount{
		...
		AccountID: &acc.ID, ExcludeFromNetWorth: true, IsArchived: true,
	})

	total, err := repo.GetTotalExcludedBalance(ctx)
	if err != nil {
...
func TestVirtualAccountRepo_GetExcludedBalanceByAccountID(t *testing.T) {
	repo, inst, acc := setupVirtualAccountTest(t)
	db := testutil.NewTestDB(t)
	ctx := context.Background()
	...
	testutil.CreateVirtualAccount(t, db, models.VirtualAccount{
		...
		AccountID: &acc2.ID, ExcludeFromNetWorth: true,
	})

	// First account should show 30000
	bal1, err := repo.GetExcludedBalanceByAccountID(ctx, acc.ID)
	if err != nil {
	...
	bal2, err := repo.GetExcludedBalanceByAccountID(ctx, acc2.ID)
	if err != nil {
		t.Fatalf("GetExcludedBalanceByAccountID: %v", err)
	}
	if bal2 != 15000 {
...
func TestVirtualAccountRepo_GetTotalExcludedBalance_Empty(t *testing.T) {
	repo, _, _ := setupVirtualAccountTest(t)
	ctx := context.Background()
...
func TestVirtualAccountRepo_CreateWithExclude(t *testing.T) {
	...
	created, err := repo.Create(ctx, models.VirtualAccount{
...
func TestVirtualAccountRepo_UpdateExclude(t *testing.T) {
	...
	created, _ := repo.Create(ctx, models.VirtualAccount{
		Name: "Building Fund", Color: "#ff0000",
		AccountID: &acc.ID, ExcludeFromNetWorth: false,
	})
	...
	fetched2, _ := repo.GetByID(ctx, created.ID)
