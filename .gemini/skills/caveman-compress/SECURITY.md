...
If `ANTHROPIC_API_KEY` is set, the skill uses the Anthropic Python SDK directly (no subprocess). If not set, it falls back to the `gemini` CLI, which uses the user's existing Gemini desktop authentication.
