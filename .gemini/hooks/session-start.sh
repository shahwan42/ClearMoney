...
# Only run in Gemini Code remote environments
if [ "${GEMINI_CODE_REMOTE:-}" != "true" ]; then
  exit 0
fi
...
echo 'export DB_URL="postgres://clearmoney:clearmoney@localhost:5432/clearmoney?sslmode=disable"' >> "$GEMINI_ENV_FILE"
