# Remote Environment Setup (Gemini CLI on the web)

## How It Works

A **SessionStart hook** (`.gemini/hooks/session-start.sh`) runs automatically when a Gemini CLI remote session starts. It:

1. Starts PostgreSQL 16 (`pg_ctlcluster 16 main start`)
2. Creates the `clearmoney` database user and database (idempotent)
3. Exports `DB_URL` with port 5432 via `$CLAUDE_ENV_FILE`
...
| Remote (Gemini CLI) | 5432 | Native PG 16 on host, started by session hook |
...
- **"connection refused on 5433"** — Hook didn't run or `DB_URL` not exported. Run manually:
  ```bash
  CLAUDE_CODE_REMOTE=true CLAUDE_ENV_FILE=/dev/null .gemini/hooks/session-start.sh
