...
2. **Create a new ticket** using the protocol in `.claude/rules/ticketing-workflow.md`:
   ...
   - Add initial progress note: `- YYYY-MM-DD: Started — [what Claude is working on]`

2.5. **Name affected user journeys**: Walk `.claude/rules/critical-paths.md`
   (CP-1..CP-6) and `.claude/rules/user-journeys.md` (J-1..J-5). Add a
...
- **Ticketing workflow** (`.claude/rules/ticketing-workflow.md`): Defines ticket mechanics (create, move, close, reject). This rule enforces **when** to use them.
- **Git workflow** (`.claude/rules/git-workflow.md`): After ticket is created, follow commit practices (show changes, ask approval, use conventional commits).
- **TDD workflow** (`.claude/rules/tdd-workflow.md`): After ticket is created, write RED tests first.
- **Delivery checklist** (`.claude/rules/delivery-checklist.md`): After ticket is created, follow feature delivery steps.
...
- **Async communication**: Ahmed and Claude can reference ticket IDs instead of relying on conversation history
