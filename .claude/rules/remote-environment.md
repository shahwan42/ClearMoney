...
A **SessionStart hook** (`.claude/hooks/session-start.sh`) runs automatically when a Claude Code remote session starts. It:
...
| Remote (Claude Code) | 5432 | Native PG 16 on host, started by session hook |
...
- **"connection refused on 5433"** — Hook didn't run or `DB_URL` not exported. Run manually:
  ```bash
  CLAUDE_CODE_REMOTE=true CLAUDE_ENV_FILE=/dev/null .claude/hooks/session-start.sh
