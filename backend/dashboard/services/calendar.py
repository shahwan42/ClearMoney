...
class CalendarService:
    ...
    def get_month_events(self, year: int, month: int) -> dict[int, list[dict[str, Any]]]:
        ...
        txs = Transaction.objects.for_user(self.user_id).filter(
        ...
        for tx in txs:
            ...
            events_by_day[day].append({
                "type": "transaction",
                "subtype": tx.type,
                "title": tx.note or (tx.category.get_display_name() if tx.category else tx.type),
