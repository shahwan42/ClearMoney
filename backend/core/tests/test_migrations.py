...
class TestAddSystemCategoriesMigration:
    ...
    def test_adds_categories_to_existing_user(self) -> None:
        ...
        cats = Category.objects.filter(user_id=user.id, name__in=NEW_NAMES)
        assert cats.count() == 4
        for cat in cats:
    ...
    def test_idempotent(self) -> None:
        user = UserFactory()
        add_system_categories(None, None)
        add_system_categories(None, None)  # run twice
        assert Category.objects.filter(user_id=user.id, name="Travel").count() == 1

    def test_correct_icons(self) -> None:
        user = UserFactory()
        add_system_categories(None, None)
        assert Category.objects.get(user_id=user.id, name="Travel").icon == "✈️"
        assert Category.objects.get(user_id=user.id, name="Cafe").icon == "☕"
        assert Category.objects.get(user_id=user.id, name="Restaurant").icon == "🍽️"
        assert Category.objects.get(user_id=user.id, name="Car").icon == "🚙"

    def test_multiple_users(self) -> None:
        user1 = UserFactory()
        user2 = UserFactory()
        add_system_categories(None, None)
        assert Category.objects.filter(user_id=user1.id, name="Travel").count() == 1
        assert Category.objects.filter(user_id=user2.id, name="Travel").count() == 1

    def test_skips_existing_same_name(self) -> None:
        """If user already has a custom 'Travel' category, don't duplicate."""
        user = UserFactory()
        CategoryFactory(user_id=user.id, name="Travel", icon="🧳", is_system=False)
        add_system_categories(None, None)
        assert (
            Category.objects.filter(user_id=user.id, name__iexact="travel").count() == 1
        )

    def test_existing_transactions_unaffected(self) -> None:
        user = UserFactory()
        cat = CategoryFactory(user_id=user.id, name="Food")
        ...
        add_system_categories(None, None)
        tx.refresh_from_db()
        assert tx.category_id == cat.id  # unchanged
